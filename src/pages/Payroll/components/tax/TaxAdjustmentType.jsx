...
const getColumns = (apiRef, onEdit, onActive) => {
    ...
    return [
        ...
        {
            field: 'name', headerName: 'Type', width: 220, hideable: false
        },
...
const DEFAULT_API = API.TaxAdjustmentType;
let editId = 0;
const _years = getYears();

export const AddTaxAdjustmentType = ({ openPopup, setOpenPopup, isEdit = false, row = null, colData = [] }) => {
...
const TaxAdjustmentType = () => {
    ...
    const { inProcess, setFile, excelData, getTemplate } = useExcelReader({
        formTemplate: excelColData.current, fileName: "TaxAdjustmentType.xlsx",
        transform: mapData,
        uniqueBy: ["name"]
    ...
    const { socketData } = useSocketIo("changeInTaxAdjustmentType", refetch);
    ...
    return (
        <>
            ...
            <AddTaxAdjustmentType colData={excelColData} openPopup={openPopup} setOpenPopup={setOpenPopup} isEdit={isEdit.current} row={row.current} />
...
export default TaxAdjustmentType;