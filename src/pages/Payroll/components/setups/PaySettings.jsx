...
const PaySettings = ({ data }) => {
    ...
    const handleAddItems = (isAllowance = true) => {
        ...
        if (isAllowance) {
            if (allowanceHeads.length === getValue().allowances.length) return;
            const newId = allowanceHeads.find(c => !disabledAllowance.includes(c.id)).id;
            setFormValue({ allowances: [...getValue().allowances, { fkAllowanceId: allowanceHeads.length ? newId : "", type: PercentageOfBasicSalary, amount: 0, percentage: 0 }] })
            disabledAllowance.push(newId);
            setDisabledAllowance([...disabledAllowance])
        }
        else {
            if (deductionHeads.length === getValue().deductions.length) return;
            const newId = deductionHeads.find(d => !disabledDeduction.includes(d.id)).id;
            setFormValue({ deductions: [...getValue().deductions, { fkDeductionId: deductionHeads.length ? newId : "", type: PercentageOfBasicSalary, amount: 0, percentage: 0 }] })
    ...
    const handleSubmit = () => {
        ...
        const dataToInsert = {
            payrollStartDay: startDay === 0 ? 0 : startDay,
            ...
            formData: [
                {
                    ...
                    dataName: "name",
                    isNone: false,
                    // defaultValue: allowanceHeads.length ? allowanceHeads[0]._id : "",
                    options: allowanceHeads,
                },
                {
                    elementType: "dropdown",
                    name: "type",
            ...
            formData: [
                {
                    ...
                    dataName: "name",
                    isNone: false,
                    // defaultValue: deductionHeads.length ? deductionHeads[0]._id : "",
                    options: deductionHeads,
                },
                {
                    elementType: "dropdown",
                    name: "type",
    ...
    const handleSetupChange = (result) => {
        ...
        if (result) {
            ...
            setDisabledAllowance(_allowances.length ? _allowances.map(d => d.payrollHeadId) : allowances.map(a => a.fkAllowanceId));
            setDisabledDeduct(_deductions.length ? _deductions.map(d => d.payrollHeadId) : deductions.map(d => d.fkDeductionId));
            setFormValue({
                startDay: payrollStartDay,
